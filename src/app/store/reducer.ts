...
export const reducer = createReducer(
  ...
  on(actions.accepted, (state, { date }) => ({
      ...state,
    dates: {[date]: { ...state.dates[date], accepted: (state.dates[date]?.accepted ?? 0) + 1 }}
  })),
  ...
  on(actions.didNotAccept, (state, { date }) => ({
    ...state,
    dates: {[date]: { ...state.dates[date], rejected: (state.dates[date]?.rejected ?? 0) + 1 }}
  })),
  ...
  on(actions.positiveBoost, (state, { date, size }) => ({
    ...state,
    dates: {[date]: { ...state.dates[date], timeSavedEstimate: (state.dates[date]?.timeSavedEstimate ?? 0) + size }}
  })),
  on(actions.negativeBoost, (state, { date, size }) => ({
    ...state,
    dates: {[date]: { ...state.dates[date], timeLostEstimate: (state.dates[date]?.timeLostEstimate ?? 0) + size }}
  })),
);
